...
@Slf4j
@Component
public class PayApiProxy {

    private final WebClient payServiceWebClient;

    public PayApiProxy(@Qualifier("payServiceWebClient") WebClient payServiceWebClient) {
    ...
    public ResponseEntity<byte[]> forward(String targetPath, HttpServletRequest request, byte[] body) {
        ...
        WebClient.RequestBodySpec spec = payServiceWebClient.method(method).uri(targetUri)
        ...
        return ProxyResponseHelper.sanitize(mono.onErrorResume(ex -> {
            log.warn("Proxy to pay-service failed: path={}, err={}", targetPath, ex.getMessage());
