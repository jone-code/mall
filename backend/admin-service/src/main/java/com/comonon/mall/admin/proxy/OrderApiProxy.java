...
@Slf4j
@Component
public class OrderApiProxy {

    private final WebClient webClient;

    public OrderApiProxy(@Qualifier("orderServiceWebClient") WebClient orderServiceWebClient) {
    ...
    public ResponseEntity<byte[]> forward(String targetPath, HttpServletRequest request, byte[] body) {
        ...
        return mono.onErrorResume(ex -> {
            log.warn("Proxy to order-service failed: path={}, err={}", targetPath, ex.getMessage());
            return Mono.just(ResponseEntity.status(502)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body("{\"code\":502,\"message\":\"order-service unavailable\"}"
