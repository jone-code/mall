...
@Slf4j
@Component
public class ReviewApiProxy {

    private final WebClient webClient;

    public ReviewApiProxy(@Qualifier("reviewServiceWebClient") WebClient reviewServiceWebClient) {
        this.webClient = reviewServiceWebClient;
    }
    ...
    public ResponseEntity<byte[]> forward(String targetPath, HttpServletRequest request, byte[] body) {
        ...
        return mono.onErrorResume(ex -> {
            log.warn("Proxy to review-service failed: path={}, err={}", targetPath, ex.getMessage());
