package com.comonon.mall.admin.proxy;
...
@Slf4j
@Component
public class ProductApiProxy {

    private final WebClient webClient;

    public ProductApiProxy(@Qualifier("productServiceWebClient") WebClient productServiceWebClient) {
    ...
    public ResponseEntity<byte[]> forward(String targetPath, HttpServletRequest request, byte[] body) {
        ...
        WebClient.RequestBodySpec spec = webClient.method(method).uri(targetUri)
        ...
        return mono.onErrorResume(ex -> {
            log.warn("Proxy to product-service failed: path={}, err={}", targetPath, ex.getMessage());
            return Mono.just(ResponseEntity.status(502)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body("{\"code\":502,\"message\":\"product-service unavailable\"}"
    ...
    private void copyHeaders(HttpServletRequest request, HttpHeaders out) {
        ...
        if (ctx != null && ctx.adminUserId() != null) {
            out.set("X-Admin-User-Id", String.valueOf(ctx.adminUserId()));
        } else {
            out.remove("X-Admin-User-Id");
        }
    }
}
